      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-4 sm:top-1/2 sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:-translate-y-1/2 z-50 w-auto sm:w-full sm:max-w-md flex items-center justify-center sm:block"
          >
            <Card className="glass border-white/20 shadow-2xl w-full max-w-sm sm:max-w-md">
              <CardContent className="p-3 sm:p-6">
                {/* Header */}
                <div className="flex items-center justify-between mb-3 sm:mb-6">
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <div className="h-6 w-6 sm:h-8 sm:w-8 liquid-gradient rounded-lg flex items-center justify-center">
                      <span className="text-white font-bold text-xs sm:text-sm">F</span>
                    </div>
                    <div>
                      <div className="font-semibold text-white text-sm sm:text-base">Guía de Fixia</div>
                ...
                {/* Content */}
                <div className="mb-3 sm:mb-6">
                  <h3 className="text-sm sm:text-lg font-semibold text-white mb-1 sm:mb-3 leading-tight">
                ...
                {currentStep === steps.length - 1 && userType === 'provider' && (
                  <div className="mb-3 sm:mb-6 p-2 sm:p-4 bg-green-500/20 border border-green-500/30 rounded-lg">
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      <MessageCircle className="w-3 h-3 sm:w-5 sm:h-5 text-green-400 mt-0.5 flex-shrink-0" />
                      <div className="text-green-100 text-xs sm:text-sm">
                        <p className="font-medium mb-0.5 sm:mb-1">¡Tip importante!</p>
                ...
                {currentStep === steps.length - 1 && userType === 'customer' && (
                  <div className="mb-3 sm:mb-6 p-2 sm:p-4 bg-blue-500/20 border border-blue-500/30 rounded-lg">
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      <Phone className="w-3 h-3 sm:w-5 sm:h-5 text-blue-400 mt-0.5 flex-shrink-0" />
                      <div className="text-blue-100 text-xs sm:text-sm">
                        <p className="font-medium mb-0.5 sm:mb-1">¿Necesitas ayuda?</p>
                        <p className="text-xs sm:text-sm leading-tight">
                ...
                {/* Navigation */}
                <div className="flex flex-col space-y-2 sm:flex-row sm:justify-between sm:space-y-0 sm:space-x-3">
                  <Button
                    variant="outline"
                    onClick={currentStep === 0 ? handleSkip : handleBack}
                    className="border-white/20 text-white hover:bg-white/10 px-3 py-2 text-xs sm:text-base w-full sm:w-auto min-h-[36px] sm:min-h-[44px] font-medium"
                  >
                    {currentStep === 0 ? (
                      <>
                  ...
                  <Button
                    onClick={handleNext}
                    className="liquid-gradient hover:opacity-90 px-3 py-2 text-xs sm:text-base w-full sm:w-auto min-h-[36px] sm:min-h-[44px] font-semibold"
                  >
